...
uint8_t AT_Init(void)
{
    ...
    // ==========================================
    // 馃洝锔? 闃插脊琛? 1锛氳繘鍏? RTOS 涓寸晫鍖猴紙鍏抽棴澶栭儴涓?鏂?锛?
    // 闃叉?㈠湪閰嶇疆瀵勫瓨鍣ㄧ殑涓�鍗婃椂锛孍SP32 鍚愭暟鎹?寮曞彂涓?鏂?椋庢毚
    // ==========================================
    taskENTER_CRITICAL();
    ...
    // ==========================================
    // 馃洝锔? 闃插脊琛? 2锛氭毚鍔涙竻闄? ORE (婧㈠嚭) 鍜? RXNE (鎺ユ敹) 鏍囧織
    // STM32 鐨勭巹瀛︼細鍏堣?? SR 瀵勫瓨鍣?锛屽啀璇? DR 瀵勫瓨鍣?锛屽氨鑳芥竻闄? ORE 閿欒??锛?
    // ==========================================
    volatile uint32_t temp_clear;
    temp_clear = USART1->SR; // 璇荤姸鎬佸瘎瀛樺櫒
    temp_clear = USART1->DR; // 璇绘暟鎹?瀵勫瓨鍣?
    (void)temp_clear;        // 楠楄繃缂栬瘧鍣?锛岄槻姝㈡姤 warning

    // ==========================================
    // 馃洝锔? 闃插脊琛? 3锛氭竻绌鸿?? ESP32 "鍨冨溇鏁版嵁" 姹℃煋鐨勮蒋浠剁紦鍐插尯
    // ==========================================
...
//    DMA_Cmd(AT_usartc.dma_rx_stream,ENABLE);
		...
		// 馃毃 蹇呴』鎶婂叏灞�鍙橀噺鏇存柊涓烘湭鐭ワ紝鍚﹀垯瀹冧細缁ф壙涓婁竴娆＄殑 OK锛?
    return AT_UNKNOWN;
}
...
uint8_t AT_HTTP_Request(const char* url, weather_info_t* info)
{
    ...
    // ?? 修复点：自己建一个专用的发送缓冲区，绝对不借用接收专用的 g_at_buf
    char cmd_buf[256]; 
	snprintf(cmd_buf, sizeof(cmd_buf), "AT+HTTPCLIENT=2,1,\"%s\",,,2", url);

    // 发送独立的 cmd_buf，接收时清空重写 g_at_buf
